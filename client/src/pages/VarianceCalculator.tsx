...
import { parseDataInput, variance, mean, formatStat } from "@/lib/statsUtils";
...
export default function VarianceCalculator() {
  ...
  const results = useMemo(() => {
    ...
    return { n: values.length, mean: m, variance: v };
  }, [values, isPopulation]);
  ...
  const handleCopy = () => {
    ...
    const text = `${label} Variance: ${formatStat(results.variance, decimals)}\nMean: ${formatStat(results.mean, decimals)}\nn: ${results.n}`;
    navigator.clipboard.writeText(text);
    toast({ title: "Copied to clipboard", description: "Variance result copied" });
  };

  const faqs = [
    {
      question: "What is variance in statistics?",
      answer: "Variance measures how far a set of numbers are spread out from their average (mean). A higher variance indicates greater spread. It is calculated as the average of the squared differences from the mean."
    },
    {
      question: "What is the difference between sample and population variance?",
      answer: "Population variance divides by n (the total number of data points), while sample variance divides by n-1 (Bessel's correction). Use sample variance when your data is a subset of a larger population, which is the most common case."
    },
    {
      question: "Why do we square the differences in variance?",
      answer: "Squaring ensures all deviations are positive (so they don't cancel out), and it gives more weight to larger deviations. The square root of variance gives us the standard deviation, which is in the same units as the original data."
    }
  ];
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col font-sans">
      <PageHead
        title="Variance Calculator - Calculate 360"
        description="Calculate sample or population variance for your dataset. Free online variance calculator with step-by-step formula explanation."
        path="/variance"
      />
      <Navigation />
          ...
          <div className="lg:col-span-2">
            <header className="mb-8">
              ...
              <p className="text-lg text-muted-foreground">
                Calculate sample or population variance to measure data spread around the mean.
              </p>
            </header>
            ...
            <div className="bg-card rounded-2xl shadow-sm border border-border p-6 md:p-8">
              ...
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
                ...
                {values.length === 1 && (
                  <p className="text-sm text-muted-foreground mt-4 text-center">Enter at least 2 data points to calculate variance.</p>
                )}
              </motion.div>
            </div>
            ...
            <div className="bg-card rounded-2xl p-6 shadow-sm border border-border">
              ...
              <ul className="space-y-3 text-sm text-muted-foreground">
                <li className="flex gap-2">
                  ...
                  Use sample variance (n-1) when analyzing a subset of data.
                </li>
                <li className="flex gap-2">
                  <span className="text-primary font-bold">•</span>
                  Population variance uses n and is for complete datasets.
                </li>
                <li className="flex gap-2">
                  <span className="text-primary font-bold">•</span>
